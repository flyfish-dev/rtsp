...
const WEBRTC_SMOOTHNESS_WINDOW_MS = 6000;
const WEBRTC_MIN_SMOOTH_FPS = 4;
const WEBRTC_MAX_SMOOTH_GAP_MS = 1800;
...
async function startFromOptions(options, eventOrigin, meta = {}) {
  ...
  await stopPlayer({ userInitiated: false, clearPlayback: false });
...
function stopPlayer(options = {}) {
  ...
  const stopPromise = stopNativeStream(streamId);
  setStats(null);
  return stopPromise;
}
...
class WebCodecsRTSPPlayer {
  constructor(canvas, hooks = {}) {
    ...
    this.maxTimestampSeen = -1;
    this.timestampResetOffset = 0;
    this.pending = [];
    this.configuredAt = 0;
    this.lastFrameAt = 0;
  ...
  handleAccessUnit(buffer) {
    ...
    // H.264 streams that contain B-frames (e.g. mediamtx republishing an
    // MP4 with IBBBP GOPs) deliver access units in decode order, but each
    // chunk's presentation timestamp (PTS) is in display order. That means
    // a B-frame's PTS legitimately goes *backwards* relative to the
    // preceding P-frame, and forcing the PTS to be monotonic here would
    // destroy WebCodecs' DPB reordering and produce the visible
    // "frame skip / frame goes backwards" stutter. So we let real PTS
    // regressions pass through. We only intervene when the regression is
    // huge (>5s backwards from the maximum we have ever seen), which is a
    // strong signal that the upstream timestamp source reset, and in that
    // case we synthesize a fresh monotonic offset rather than rewriting
    // individual chunks.
    if (this.maxTimestampSeen < 0) {
      this.maxTimestampSeen = timestamp;
    } else {
      const HARD_REGRESSION_US = 5_000_000;
      if (timestamp + HARD_REGRESSION_US < this.maxTimestampSeen) {
        const newOffset = this.timestampResetOffset + (this.maxTimestampSeen - timestamp) + 33_333;
        logPlayer('warn', 'access unit timestamp reset detected', {
          codec: this.codec,
          chunkTimestamp: timestamp,
          maxSeen: this.maxTimestampSeen,
          newOffset
        });
        this.timestampResetOffset = newOffset;
      }
      timestamp += this.timestampResetOffset;
      if (timestamp > this.maxTimestampSeen) this.maxTimestampSeen = timestamp;
    }
...
class WebRTCTransportPlayer {
  ...
  drawVideo(video) {
    ...
    const now = performance.now();
    if (this.lastFrameAt) {
      this.maxFrameGap = Math.max(this.maxFrameGap, now - this.lastFrameAt);
    }
  ...
  startWatchdog() {
    ...
    this.watchdog = setInterval(() => {
      ...
      if (this.windowStartedAt && now - this.windowStartedAt >= WEBRTC_SMOOTHNESS_WINDOW_MS) {
        const elapsed = now - this.windowStartedAt;
        const fps = this.windowFrames * 1000 / elapsed;
        const maxGap = this.maxFrameGap;
        logPlayer('info', 'WebRTC smoothness sample', {
          frames: this.windowFrames,
          fps: Number(fps.toFixed(2)),
          maxGap: Math.round(maxGap)
        });
        if (fps < WEBRTC_MIN_SMOOTH_FPS || maxGap > WEBRTC_MAX_SMOOTH_GAP_MS) {
          this.notifyRecoverable('webrtc-poor-smoothness', {
            frames: this.windowFrames,
            fps: Number(fps.toFixed(2)),
            maxGap: Math.round(maxGap)
          });
