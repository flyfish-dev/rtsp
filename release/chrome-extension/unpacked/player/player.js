...
const DEFAULT_ALLOWED_ORIGINS = ['http://localhost', 'http://127.0.0.1', 'https://localhost'];
let player = null;
...
async function startFromOptions(options, eventOrigin) {
  ...
  if ((mediaTransport === 'auto' || mediaTransport === 'webrtc') && await canAttemptWebRTC(codec)) {
...
class WebCodecsRTSPPlayer {
  ...
  connect(wsUrl) {
    ...
    this.ws.onopen = () => this.hooks.onStatus?.('已连接本地网关，等待视频参数…', '');
    this.ws.onerror = () => this.hooks.onStatus?.('WebSocket 连接错误', 'error');
    this.ws.onclose = () => {
      if (!this.closed) this.hooks.onStatus?.('视频连接已关闭', 'error');
    };
...
class WebRTCTransportPlayer {
  ...
  async connect(url, codec) {
    ...
    this.pc.ontrack = (event) => {
      ...
      this.hooks.onStatus?.(`WebRTC 已就绪：${codec || 'auto'}`, 'ok');
      this.hooks.onReady?.();
    };
    ...
    await delay(2600);
    if (this.gotTrack) return true;
    this.hooks.onStatus?.('WebRTC 未收到视频，回退 WebSocket。');
    this.close();
    return false;
  }
