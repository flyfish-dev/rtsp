(() => {
  ...
  async function getAllowedOrigins() {
    ...
    try {
      const data = await chrome.storage.local.get({ allowedOrigins: ['http://localhost', 'http://127.0.0.1', 'https://localhost', 'https://rtsp-roan.vercel.app'] });
      allowedOriginsCache = data.allowedOrigins || [];
  ...
  async function mount(el) {
    ...
    mounted.set(el, { shadow, wrap, iframe: null });
  ...
  chrome.storage.onChanged.addListener((changes, area) => {
    if (area === 'local' && changes.allowedOrigins) {
      allowedOriginsCache = changes.allowedOrigins.newValue || [];
    }
  });
