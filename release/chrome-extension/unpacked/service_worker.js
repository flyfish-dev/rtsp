...
const NATIVE_DEBUG_KEY = 'rtspNativeDebug';
...
async function sendNative(message) {
  ...
  if (typeof request.debug !== 'boolean') {
    request.debug = await nativeDebugEnabled();
  }
  const maxAttempts = QUIET_NATIVE_TYPES.has(message?.type) ? 1 : 2;
  let last = null;
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    last = await sendNativeOnce(request, attempt);
...
async function nativeDebugEnabled() {
  const data = await chrome.storage.local.get({ [NATIVE_DEBUG_KEY]: false });
  return Boolean(data[NATIVE_DEBUG_KEY]);
}
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  (async () => {
    ...
    if (message.type === 'getNativeDebug') {
      sendResponse({ ok: true, enabled: await nativeDebugEnabled() });
      return;
    }

    if (message.type === 'setNativeDebug') {
      const enabled = Boolean(message.enabled);
      await chrome.storage.local.set({ [NATIVE_DEBUG_KEY]: enabled });
      await addLog('info', 'extension', `native debug logging ${enabled ? 'enabled' : 'disabled'}`);
    ...
    if (message.type === 'stopStream') {
      ...
      sendResponse(await sendNative({ type: 'stopStream', streamId }));
      return;
    }
    ...
    if (message.type === 'ensureAllowedOrigin') {
      const origin = normalizeAllowedOrigin(message.origin || originFromSender(sender));
      if (!origin) {
        sendResponse({ ok: false, error: 'origin must be http:// or https://' });
        return;
      }
      const data = await chrome.storage.local.get({ allowedOrigins: defaultAllowedOrigins() });
      const current = withDefaultAllowedOrigins(data.allowedOrigins);
      const alreadyAllowed = current.some((pattern) => originMatches(pattern, origin));
      const allowedOrigins = alreadyAllowed ? current : Array.from(new Set([...current, origin]));
      if (!alreadyAllowed) {
        await chrome.storage.local.set({ allowedOrigins });
        await addLog('info', 'extension', 'page origin allowed automatically', { origin, sender: senderLabel(sender) });
      }
      sendResponse({ ok: true, origin, originAllowed: true, added: !alreadyAllowed, allowedOrigins });
      return;
    }
...
function originFromSender(sender) {
  ...
  if (sender?.url) return sender.url;
