...
const DEFAULTS = ['http://localhost', 'http://127.0.0.1', 'https://localhost', 'https://rtsp-roan.vercel.app'];
...
async function addOrigin() {
  ...
  const { allowedOrigins = DEFAULTS } = await chrome.storage.local.get({ allowedOrigins: DEFAULTS });
  const next = Array.from(new Set([...allowedOrigins, value]));
  await chrome.storage.local.set({ allowedOrigins: next });
  originInput.value = '';
  renderOrigins(next);
}
