...
async function checkNative() {
  ...
  if (resp?.ok) {
    nativeStatus.textContent = `Native Runtime 正常，Gateway 端口：${resp.port}，版本：${resp.gatewayVersion || resp.version || '-'}`;
...
async function refreshLogs() {
  ...
  const [diagnostics, extensionLogs] = await Promise.all([
    chrome.runtime.sendMessage({ type: 'getDiagnostics', maxBytes: 192 * 1024 }).catch((err) => ({ ok: false, error: err?.message || String(err) })),
  ...
  if (diagnostics?.ok) {
    diagnosticStatus.textContent = diagnostics.healthy
      ? `Native 正常：PID ${diagnostics.pid || '-'}，端口 ${diagnostics.port || '-'}，版本 ${diagnostics.gatewayVersion || diagnostics.version || '-'}`
