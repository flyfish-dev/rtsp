...
如果 VLC/FFmpeg 能播放但浏览器无画面，重点看日志里的 `h264 annexb-in-rtp`、`h264 fu-a annexb-fragment`、`webrtc repacketize`。从 `0.1.14` 开始，Native 会兼容这类非标准 H.264 RTP：WebSocket/WebCodecs 路径拆出真正的 NALU，WebRTC 路径在不转码的前提下重新封装 RTP，提高现场摄像头兼容性。
