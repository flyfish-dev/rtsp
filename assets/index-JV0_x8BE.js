...
The latency policy is minimal buffering with fast recovery. WebRTC forwards RTP first; the WebSocket fallback sends Annex-B access units only. H.264 startup detects IDR, non-IDR I/SI slices, intra AUDs, and SPS/PPS refresh frames. Decoder errors rebuild the decoder first, then restart the stream if failures repeat.
...
- \`first access unit ... key=false bytes=...\` while VLC plays: some cameras encode startup pictures as non-IDR I-slices or as non-IDR refresh frames after SPS/PPS. \`0.1.8\` treats I/SI slices, intra AUDs, and SPS/PPS + VCL access units as key/startup frame candidates.
- \`DESCRIBE/SETUP/PLAY status 401\`: the camera rejected RTSP authentication. Verify username, password, and live-view permissions. \`0.1.8\` adds Digest URI compatibility retries; if it still fails, try removing URL parameters such as \`?transportmode=...&profile=...\`.
...
低延迟策略是“少缓存、快起播、可恢复”：WebRTC 直转 RTP 优先，WebSocket 回退只传 Annex-B access unit；H.264 起播会识别 IDR、non-IDR I/SI slice、AUD intra 以及 SPS/PPS 刷新帧，解码异常时先重建解码器，连续失败再自动重拉流。
...
- \`first access unit ... key=false bytes=...\` 但 VLC 能播放：部分摄像头会把可起播帧编码为 non-IDR I-slice，或在 SPS/PPS 参数集后给出非 IDR 刷新帧。\`0.1.8\` 会把 I/SI slice、AUD intra、SPS/PPS + VCL access unit 都作为关键帧/起播帧候选处理。
- \`DESCRIBE/SETUP/PLAY status 401\`：摄像头拒绝 RTSP 鉴权。先确认 URL 中用户名密码正确，再检查账号是否有实时预览权限。\`0.1.8\` 已增加 Digest URI 兼容重试；如果仍是 401，优先尝试去掉 \`?transportmode=...&profile=...\` 这类 URL 参数。
