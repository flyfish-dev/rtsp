// RTSP SDK 0.1.1 - generated by scripts/build-sdk.mjs
...
export const RTSP_PLAYER_VERSION = "0.2.0";
...
export function defineRTSPPlayer(tagName = globalConfig.tagName || DEFAULT_TAG_NAME, options = {}) {
  ...
  class RTSPPlayerElement extends HTMLElement {
    ...
    _renderExtension() {
      ...
      if (!origin) {
        host.innerHTML = `<div class="missing">RTSP: missing Chrome extension id.</div>`;
        return;
      }
    ...
    async _startDesktop() {
      ...
      if ((mediaTransport === "auto" || mediaTransport === "webrtc") && await this._canAttemptWebRTC(codec)) {
        const started = await this._startWebRTC(bridge, url, codec);
        if (started) return;
      }
      await this._startWebSocket(bridge, url, codec);
    }
    ...
    async _startWebRTC(bridge, url, codec) {
      if (!bridge.createWebRTCOffer) return false;
      ...
      pc.ontrack = (event) => {
        ...
        this._attachVideoStream(stream);
    ...
    async _startWebSocket(bridge, url, codec) {
...
class AnnexBWebCodecsPlayer {
  ...
  connect(wsUrl) {
    ...
    this.ws.onclose = () => {
      if (!this.closed) this.hooks.onStatus?.("Video connection closed.", "error");
    };
  ...
  async handleMessage(data) {
    if (typeof data === "string") {
      ...
      else if (msg.type === "error") this.hooks.onError?.(msg.error || "RTSP error");
      return;
    }
  ...
  async configure(codec) {
    ...
    this.decoder = new VideoDecoder({
      ...
      error: (err) => this.hooks.onError?.(err?.message || String(err)),
    });
