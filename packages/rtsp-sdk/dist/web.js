...
export const RTSP_PLAYER_VERSION = "0.1.7";
...
class AnnexBWebCodecsPlayer {
  ...
  async configure(codec) {
    ...
    this.waitingKeyLogged = false;
    this.waitingKeyStatusAt = 0;
    this.hooks.onStatus?.(`Decoder ready: ${codec}. Waiting for first keyframe...`, "ok");
  }
  ...
  handleAccessUnit(buffer) {
    ...
    if (!key && !this.gotKey) {
      ...
      if (!this.waitingKeyLogged || now - this.waitingKeyStatusAt > 5000) {
        ...
        this.hooks.onStatus?.("Waiting for the first camera keyframe (IDR)...");
      }
      return;
    }
    if (key && !this.gotKey) {
